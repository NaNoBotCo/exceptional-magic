...
#
#   ./publish.sh            # figures, data, pages, into docs/
set -euo pipefail
cd "$(dirname "$0")"
export PYTHONUTF8=1
SITE_URL="${SITE_URL:-https://nanobotco.github.io/exceptional-magic}"
...
python3 tools/algebra.py >/dev/null || { echo "REFUSED: an algebra check failed"; exit 5; }
python3 tools/roots.py >/dev/null || { echo "REFUSED: a root-system check failed"; exit 5; }
